...
class Melon:
    ...
    def is_sellable(self, shape_rating, color_rating, field):
        ...
        if int(shape_rating) > 5:
            if int(color_rating) > 5:
                if int(field) != 3:
                    return True
        else:
            return False
...
def make_melons(melon_types):
    ...
    melon_8 = Melon(melons_by_id['musk'], '6', '7', '44', 'Michael')
...
def get_sellability_report(melons):
    ...
    for melon in melons:
        ...
        sellable = melon.is_sellable(melon.shape_rating, melon.color_rating, melon.field)
        if sellable == True:
            print(f"Harvested by {harvester} from Field {field} (CAN BE SOLD)")
        else:
            print(f"Harvested by {harvester} from Field {field} (NOT SELLABLE)")
